...
- В лабе не указано, какой SSH ключ нужно создавать на Github (Authentication или Signing). Пришлось самостоятельно искать ответ и оказалось, что необходимо создавать только ключ Authentication, а также нужно создать ключ GPG, т.к. Signing key создан только для подписания коммитов (pull/push не работает при нем), что не было указано в файле с заданием. 