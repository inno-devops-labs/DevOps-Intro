...
I used the VirtualBox shared folder (the default). It needs zero host setup, works out of the box with bento's pre-installed Guest Additions, and gives live two-way sync of `./app`. The trade-off: VirtualBox shared folders are slower for heavy file I/O and have occasional permission/symlink quirks. NFS is much faster for big trees but needs an NFS server on the host (not native on Windows) and `sudo`; rsync is fast inside the guest but is a one-shot host→guest copy (no live edits unless you run `rsync-auto`); SMB needs Windows share credentials. For a small source folder, the VirtualBox driver is the simplest adequate choice.
...
the network. (Host-only would also isolate it, but NAT + port-forward was easier)
...
is the lowest-overhead option. (If provisioning grew to many roles, I'd have to switch to
`ansible_local`)
...
`vagrant up`, on any machine, installs the identical toolchain, so a
...
laptop is lost or stolen, or the VM directory is deleted, the snapshot dies with the VM.
It protects you from in-guest mistakes, not from hardware loss, host compromise, or accidental deletion of the VM itself as those need copies on other media/hosts (backups).
...
The VM numbers below are measured from running Vagrant box. The container
column is the QuickNotes image from Lab 6.
...
something that genuinely expects a whole machine.
