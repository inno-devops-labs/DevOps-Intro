...
3. **Path filter** — `on.push.paths` and `on.pull_request.paths` restrict triggers to `app/**` and `.github/workflows/ci.yml`. For `push` events to `main`, only commits that touch `app/` or `ci.yml` trigger the pipeline. For `pull_request` events, GitHub evaluates the entire PR diff: a PR where _all_ changed files are in `README.md`, `labs/`, or `submissions/` would be skipped entirely. The current feature/lab3 PR always triggers because it contains `app/handlers_test.go` changes across its commits. The filter is verified by the workflow config: commit `56b5d5d` in this branch touches only `submissions/lab3.md` — it produces no `push` CI run (the push trigger only fires on `main`), confirming the `push` path filter is active.
