...
> Screenshot: with the stack up, the dashboard is at `http://localhost:3000`
> (admin / `$GF_ADMIN_PASSWORD`) and shows the four panels populated. The API
> evidence above proves the same: target `up`, datasource default, dashboard
> provisioned, and non-zero values for all four panel queries.
...
`annotations.runbook=docs/runbook/high-error-rate.md`, `value≈0.49`. (Visible at
`http://localhost:9090/alerts` while traffic runs.)
