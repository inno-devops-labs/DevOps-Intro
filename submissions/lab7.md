...
> How it was run: this Windows host can't be an Ansible control node, and WSL
> needs Hyper-V — which is off so the VirtualBox VM can run. So I ran Ansible
> **inside the VM against itself** (`ansible_connection=local`, see
> [`inventory-local.ini`](../ansible/inventory-local.ini)) — the same local model
> the Bonus `ansible-pull` loop uses. The committed `inventory.ini` is the normal
> push-from-host inventory (port 2222 from `vagrant ssh-config`) for graders on a
...
```
(Binary built in the VM: `statically linked, stripped` ELF, 5.8 MB. Ansible core 2.16.3.)
...
`listen_addr`; (2) **group_vars / host_vars** for per-environment values (e.g. a
prod vs the VM); (3) **playbook `vars:` or `-e` extra-vars** for explicit
per-run overrides (extra-vars win over everything). For this small lab I keep them
...
fine — but the diff reveals the change is wrong (a typo'd address, a secret
